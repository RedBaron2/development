...
function global:au_BeforeUpdate {
  ...
  if ($global:au_Force -eq $true) {
	Set-ReadMeFile -keys "fileType,Vendor,JVM_Type,PackageName,HotFix" -new_info "$($Latest.fileType),AdoptOpenJDK,$jvm,$($Latest.PackageName),**_Updated_Version_**"
  } else {
	Set-ReadMeFile -keys "fileType,Vendor,JVM_Type,PackageName,HotFix" -new_info "$($Latest.fileType),AdoptOpenJDK,$jvm,$($Latest.PackageName),&nbsp;"
  }
...
[bool]$FixVersion = $false, # optional bool for FixVersion
...
if ($FixVersion -eq $true) {
Write-Verbose "Forced Versioning ${type}${number} ${jvm}"
  ...
  $fixed = ""
	...
	@{
        Title           = "AdoptOpenJDK ${type}${number} ${jvm} ${packageVersion}${fixed}"
...
# This will need the au_Force variable set to $true in addition to changing hotfix
# Change to true if HotFix Versioning is needed
$hotfix = $global:au_Force
		...
		foreach ( $jvm in $jvms ) {
			foreach ( $build in $builds ) {        
				...
				if ( $number -eq $numbers[-1] ) { 
					...
					$streams.Add( "$($type)$($number)_$($jvm)_$($build)_Latest" , ( Get-OpenSourceJDK -number $number -type $type -jvm $jvm -OS $os -release $build -dev_name $name -ext -FixVersion $hotfix ) )
				} 
				...
				$streams.Add( "$($type)$($number)_$($jvm)_$($build)" , ( Get-OpenSourceJDK -number $number -type $type -jvm $jvm -OS $os -release $build -dev_name $name -ext -FixVersion $hotfix ) )        
			}
		}
	}
}
